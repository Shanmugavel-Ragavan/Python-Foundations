...
print("First character (s[0]):", s[0])   # 's'
print("Last character (s[-1]):", s[-1])  # 'e'

# Slicing: s[start:end] (end not included)
print("s[0:2] ->", s[0:2])   # 'sa' (index 0 and 1)
print("s[:5] ->", s[:5])     # 'sampl' (start from 0 up to 4)
print("s[1:] ->", s[1:])     # 'ample' (from index 1 to end)
print("s[-2:-1] ->", s[-2:-1]) # 'l' (second last character)
print("s[:-1] ->", s[:-1])   # 'sampl' (everything except last character)

# Reverse the string
print("s[::-1] ->", s[::-1]) # 'elpmas' (reverses the string)